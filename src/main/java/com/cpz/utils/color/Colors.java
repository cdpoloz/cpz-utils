 ...
 * <p>
 * This class builds, adjusts, interpolates, and reads packed color values. It
 * has no dependency on Processing, AWT, CSS parsing, color spaces, gradients, or
...
public final class Colors {
     ...
     * <p>
     * The interpolation factor is clamped to {@code 0..1}. Alpha, red, green,
     * and blue are interpolated separately and rounded to byte channel values.
     *
     * @param c1 first packed {@code 0xAARRGGBB} color
     * @param c2 second packed {@code 0xAARRGGBB} color
     * @param t  interpolation factor, clamped to {@code 0..1}
     * @return interpolated packed {@code 0xAARRGGBB} color
     */
    public static int lerpColor(int c1, int c2, float t) {
        ...
        int a = Math.round(a1 + (a2 - a1) * clampedT);
        int r = Math.round(r1 + (r2 - r1) * clampedT);
        int g = Math.round(g1 + (g2 - g1) * clampedT);
        int b = Math.round(b1 + (b2 - b1) * clampedT);
